...
exports.renderDashboard = (req, res) => {
  ...
  const startOfLastMonth = new Date(yyyy, mm - 1, 1).toISOString().slice(0, 10);
  const endOfLastMonth = new Date(yyyy, mm, 0).toISOString().slice(0, 10);
  ...
  db.all(expensesQuery, [userId], (err, items) => {
    ...
    data.thisMonthTotal = expenseItems
      .filter((e) => e.spent_at >= startOfMonth && e.spent_at <= endOfMonth)
    ...
    const lastMonthTotal = expenseItems
      .filter(
        (e) => e.spent_at >= startOfLastMonth && e.spent_at <= endOfLastMonth
      )
      .reduce((sum, e) => sum + e.amount, 0);
